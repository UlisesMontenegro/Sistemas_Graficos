        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));
            ...
            if(textureColor.y > textureColor.x && textureColor.y > textureColor.z){
                position = normal*1.02;
            }else{
                position+=normal*(1.0+sin(uv.x*18.0*PI+time*50.0)+sin(uv.y*18.0*PI+time*50.0))*0.01;
            }
