        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));
            ...
            position.y=(0.01/(sqrt((position.x*position.x)+(position.z*position.z))+0.05))*cos(2.0*PI*(sqrt((position.x*position.x)+(position.z*position.z))-time*10.0))*cos(2.0*PI*(sqrt((position.x*position.x)+(position.z*position.z))-time*10.0));
